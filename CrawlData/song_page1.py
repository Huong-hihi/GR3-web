...
def Data(url, mydb):
    ...
    cur = mydb.cursor()
    sql = "INSERT INTO singer_test (name, full_name, nickname, birthday, nation, prize, information, url, home_town ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    if (sn != None) :
        ...
        for i in temp:
            ...
            if a == 'Ngày sinh:':
                date = i.text.split(':')[1]
                date1 = date.replace("-", "/")
                test = str(date1).strip()
                def try_parsing_date(text):
                    for fmt in ('%Y-%m-%d', '%d.%m.%Y', '%d/%m/%Y', '%Y'):
                        try:
                            dateh = datetime.strptime(text, fmt)
                            print(dateh)
                            return dateh
                        except ValueError:
                            pass
                return try_parsing_date(test)
                # date = date.replace("-", "/").split('/')
               
                # datetime_object = datetime.strptime(test,'%d/%m/%Y');
                # print(datetime_object)
                # date = date.replace("-", "/").split('/')
                # if len(date) == 1 and date[0] != 'Đang cập nhật':
                #     x = date[0]+'-01-01'
                #     print(x)
                #     return x
                # x = '-'.join(date[::-1]).replace(" ", "")
               # return x
    return None
#Lấy quê quán 
